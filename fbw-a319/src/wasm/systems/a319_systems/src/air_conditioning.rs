...
impl PackFlowControllers<3, 2> for A319AirConditioning {
...
impl SimulationElement for A319AirConditioning {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
        self.a319_cabin.accept(visitor);
...
impl A319Cabin {
    ...
    const A319_CABIN_VOLUME_CUBIC_METER: f64 = 200.; // m3
    const A319_COCKPIT_VOLUME_CUBIC_METER: f64 = 10.; // m3
        ...
        Self {
            cabin_zone: [
                CabinZone::new(
                    context,
                    ZoneType::Cockpit,
                    Volume::new::<cubic_meter>(Self::A319_COCKPIT_VOLUME_CUBIC_METER),
                    2,
                    None,
                ),
                CabinZone::new(
                    context,
                    ZoneType::Cabin(1),
                    Volume::new::<cubic_meter>(Self::A319_CABIN_VOLUME_CUBIC_METER / 2.),
                    0,
                    Some([(1, 6), (7, 13)]),
                ),
                CabinZone::new(
    ...
    fn update(
        ...
        let flow_rate_per_cubic_meter: MassRate = MassRate::new::<kilogram_per_second>(
            pack_flow.pack_flow().get::<kilogram_per_second>()
                / (Self::A319_CABIN_VOLUME_CUBIC_METER + Self::A319_COCKPIT_VOLUME_CUBIC_METER),
        );
...
impl CabinTemperature for A319Cabin {
...
impl SimulationElement for A319Cabin {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
