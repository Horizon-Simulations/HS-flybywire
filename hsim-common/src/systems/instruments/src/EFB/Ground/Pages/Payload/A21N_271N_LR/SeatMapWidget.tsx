...
import { CanvasConst, SeatConstantsA21N, SeatInfo, PaxStationInfo, SeatType, RowInfo } from '../Seating/Constants';
...
export const SeatMapWidget: React.FC<SeatMapProps> = ({ seatMap, desiredFlags, activeFlags, canvasX, canvasY, theme, isMainDeck, onClickSeat }) => {
    ...
    const addXOffsetRow = useMemo(() => (xOff: number, rowInfo: RowInfo, station: number, row: number) => {
        ...
        if (row !== 0 || station !== 0) {
            xOff += (SeatConstantsA21N[seatType].padX + SeatConstantsA21N[seatType].len);
        }
        return xOff;
    }, [ctx]);
    ...
    const addYOffsetSeat = useMemo(() => (yOff: number, station: number, row: number, seat: number) => {
        ...
        if (seat !== 0) {
            yOff += (SeatConstantsA21N[seatType].padY + SeatConstantsA21N[seatType].wid);
        }
        return yOff;
    }, [ctx]);
    ...
    const drawRow = (x: number, deck: number, station: number, rowI: number, rowInfo: RowInfo, seatId: number) => {
        ...
        for (let seat = 0, yOff = 0; seat < seatsInfo.length; seat++) {
            ...
            xYMap[station][seatId] = [x + SeatConstantsA21N[seatsInfo[seat].type].imageX / 2, yOff + SeatConstantsA21N[seatsInfo[seat].type].imageY / 2, deck];
            setXYMap(xYMap);
            drawSeat(x, yOff, seatsInfo[seat].type, SeatConstantsA21N[seatsInfo[seat].type].imageX, SeatConstantsA21N[seatsInfo[seat].type].imageY, station, seatId++);
        }
    };
