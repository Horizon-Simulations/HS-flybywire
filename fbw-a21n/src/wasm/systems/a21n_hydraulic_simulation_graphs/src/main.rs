...
use a21n_systems::hydraulic::A21NHydraulicCircuitFactory;
...
fn main() {
    ...
    let path = "./src/systems/a21n_hydraulic_simulation_graphs/";
...
fn blue_circuit_epump(path: &str) {
    ...
    let mut test_bed = SimulationTestBed::new(|context| {
        ...
        A21NSimpleMainElecHydraulicsTestAircraft::new(context, hyd_loop, pump)
    });
...
fn hydraulic_loop(context: &mut InitContext, loop_color: HydraulicColor) -> HydraulicCircuit {
    match loop_color {
        HydraulicColor::Yellow => {
            A21NHydraulicCircuitFactory::new_yellow_circuit(context, Volume::default())
        }
        HydraulicColor::Blue => A21NHydraulicCircuitFactory::new_blue_circuit(context),
        HydraulicColor::Green => A21NHydraulicCircuitFactory::new_green_circuit(context),
    }
}
...
struct A21NSimpleMainElecHydraulicsTestAircraft {
...
impl A21NSimpleMainElecHydraulicsTestAircraft {
    fn new(
...
impl Aircraft for A21NSimpleMainElecHydraulicsTestAircraft {
...
impl SimulationElement for A21NSimpleMainElecHydraulicsTestAircraft {
