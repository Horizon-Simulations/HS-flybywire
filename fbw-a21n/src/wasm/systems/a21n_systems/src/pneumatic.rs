...
impl A21NPneumatic {
    ...
    pub(crate) fn update(
        ...
        overhead_panel: &A21NPneumaticOverheadPanel,
...
impl PneumaticBleed for A21NPneumatic {
...
impl EngineStartState for A21NPneumatic {
...
impl PackFlowValveState for A21NPneumatic {
...
impl SimulationElement for A21NPneumatic {
...
impl ReservoirAirPressure for A21NPneumatic {
...
pub struct A21NPneumaticOverheadPanel {
...
impl A21NPneumaticOverheadPanel {
    pub fn new(context: &mut InitContext) -> Self {
        A21NPneumaticOverheadPanel {
...
impl EngineBleedPushbutton<2> for A21NPneumaticOverheadPanel {
    fn engine_bleed_pushbuttons_are_auto(&self) -> [bool; 2] {
        [self.engine_1_bleed.is_auto(), self.engine_2_bleed.is_auto()]
    }
}
impl SimulationElement for A21NPneumaticOverheadPanel {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
...
/// This is a unique valve (and specific to the A21N probably) because it is controlled by two motors. One for manual control and one for automatic control
...
mod tests {
    ...
    use super::{A21NPneumatic, A21NPneumaticOverheadPanel};

    struct TestAirConditioning {
        a21n_air_conditioning_system: AirConditioningSystem<3, 2, 2>,
    ...
    impl TestAirConditioning {
        fn new(context: &mut InitContext) -> Self {
            ...
            Self {
                a21n_air_conditioning_system: AirConditioningSystem::new(
                    context,
                    cabin_zones,
        ...
        fn update(
            ...
            self.a21n_air_conditioning_system.update(
                context,
                &self.adirs,
                &self.test_cabin,
                engines,
    ...
    struct PneumaticTestAircraft {
        pneumatic: A21NPneumatic,
        air_conditioning: TestAirConditioning,
        lgciu: TestLgciu,
        apu: TestApu,
        engine_1: LeapEngine,
        engine_2: LeapEngine,
        pneumatic_overhead_panel: A21NPneumaticOverheadPanel,
    ...
    impl PneumaticTestAircraft {
        fn new(context: &mut InitContext) -> Self {
            Self {
                pneumatic: A21NPneumatic::new(context),
                air_conditioning: TestAirConditioning::new(context),
                lgciu: TestLgciu::new(true),
                apu: TestApu::new(),
                engine_1: LeapEngine::new(context, 1),
                engine_2: LeapEngine::new(context, 2),
                pneumatic_overhead_panel: A21NPneumaticOverheadPanel::new(context),
    ...
    fn full_graphing_test() {
        ...
        if fs::create_dir_all("../a21n_pneumatic_simulation_graph_data").is_ok() {
            let mut file = File::create("../a21n_pneumatic_simulation_graph_data/generic_data.txt")
    ...
    fn hydraulic_reservoir_pressurization_graphs() {
        ...
        if fs::create_dir_all("../a21n_pneumatic_simulation_graph_data").is_ok() {
            let mut file = File::create(
                "../a21n_pneumatic_simulation_graph_data/hydraulic_reservoir_pressures_data.txt",
            )
            .expect("Could not create file");
    ...
    fn pack_pressurization_graphs() {
        ...
        if fs::create_dir_all("../a21n_pneumatic_simulation_graph_data").is_ok() {
            let mut file =
                File::create("../a21n_pneumatic_simulation_graph_data/pack_pressures_data.txt")
                    .expect("Could not create file");
