...
export class AircraftVersionChecker {
    ...
    public static async getBuildInfo(): Promise<BuildInfo> {
        ...
        await fetch('/VFS/A321EC_build_info.json').then((response) => {
    ...
    private static async initialize() {
        this.releaseInfo = await GitVersions.getReleases('headwindsim', 'A321EC', false, 0, 1);
        this.newestCommit = await GitVersions.getNewestCommit('headwindsim', 'A321EC', 'main');
        this.newestExpCommit = await GitVersions.getNewestCommit('headwindsim', 'A321EC', 'experimental');
