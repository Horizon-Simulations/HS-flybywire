...
const TrafficIndicator: FC<TrafficProp> = memo(({ x, y, relativeAlt, vertSpeed, intrusionLevel, trafficHeading, ownHeading , trafficScale}) => {
    ...
    return (
        <>
            <Layer x={x} y={y}>
                <g>
                    {intrusionLevel === TaRaIntrusion.TRAFFIC && <image width={trafficScale} height={trafficScale} xlinkHref="/Images/A321EC/ND/TRAFFIC_NORMAL.svg" transform={`rotate(${trafficRotation} ${trafficScale / 2} ${trafficScale / 2})`} />}
                    {intrusionLevel === TaRaIntrusion.PROXIMITY && <image width={trafficScale} height={trafficScale} xlinkHref="/Images/A321EC/ND/TRAFFIC_PROXIMITY.svg" transform={`rotate(${trafficRotation} ${trafficScale / 2} ${trafficScale / 2})`} />}
                    {intrusionLevel === TaRaIntrusion.TA && <image width={trafficScale} height={trafficScale} xlinkHref="/Images/A321EC/ND/TRAFFIC_TA.svg" transform={`rotate(${trafficRotation} ${trafficScale / 2} ${trafficScale / 2})`} />}
                    {intrusionLevel === TaRaIntrusion.RA && <image width={trafficScale} height={trafficScale} xlinkHref="/Images/A321EC/ND/TRAFFIC_RA.svg" transform={`rotate(${trafficRotation} ${trafficScale / 2} ${trafficScale / 2})`} />}
                </g>
...
const TrafficIndicatorDebug: FC<TrafficPropDebug> = memo(({ x, y, relativeAlt, vertSpeed, intrusionLevel, ID, hidden, seen, raTau, taTau, vTau, closureRate, closureAccel }) => {
    ...
    return (
        <>
            <Layer x={x} y={y}>
                {intrusionLevel === TaRaIntrusion.TRAFFIC && <image opacity={hidden ? 0.125 : 1.0} x={0} y={0} width={45} height={32} xlinkHref="/Images/A321EC/ND/TRAFFIC_NORMAL.svg" />}
                {intrusionLevel === TaRaIntrusion.PROXIMITY && <image opacity={hidden ? 0.125 : 1.0} x={0} y={0} width={45} height={32} xlinkHref="/Images/A321EC/ND/TRAFFIC_PROXIMITY.svg" />}
                {intrusionLevel === TaRaIntrusion.TA && <image opacity={hidden ? 0.125 : 1.0} x={0} y={0} width={45} height={32} xlinkHref="/Images/A321EC/ND/TRAFFIC_TA.svg" />}
                {intrusionLevel === TaRaIntrusion.RA && <image opacity={hidden ? 0.125 : 1.0} x={0} y={0} width={45} height={32} xlinkHref="/Images/A321EC/ND/TRAFFIC_RA.svg" />}
                <g>
