 ...
 * Sub-Indexes: 0 to 16 represent gross weight (t) in 2.5t steps from 39 to 69
 */
const to = [
    [
        () => 125, //40
...
function _correctMass2(m) {
    ...
    const max = 80;
    const step = 5;
    const weight = m < min ? min : m; // in A318 weight can be less than 40
    return Math.ceil(((weight > max ? max : weight) - min) / step);
}
