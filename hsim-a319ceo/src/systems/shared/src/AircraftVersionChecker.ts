...
/**
 * Contains the a319hs_build_info.json file's information in a structured way.
 */
...
export class AircraftVersionChecker {
    ...
    public static async checkVersion(): Promise<boolean> {
        ...
        // Retrieve the version info from a319hs_build_info.json and GitHub
    ...
    /**
     * Reads the a319hs_build_info.json file and returns the data a BuildInfo object.
    ...
    public static async getBuildInfo(): Promise<BuildInfo> {
        ...
        await fetch('/VFS/a319hs_build_info.json').then((response) => {
     ...
     *
     * @param versionString as provided by the a319hs_build_info.json file.
    ...
    /**
     *Set the L:A319HS_OUTDATED_VERSION flag to true or false
     *
     * @param b
     * @private
     */
    private static setOutdatedVersionFlag(b: boolean) {
        SimVar.SetSimVarValue('L:A319HS_OUTDATED_VERSION', 'Bool', b ? 1 : 0);
    }
