...
export class N1Limit extends DisplayComponent<N1LimitProps> {
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on('engine2Fadec').whenChanged().handle((f) => {
            this.engine2Fadec = f;
        });
        ...
        sub.on('sat').whenChanged().handle((sat) => {
            this.sat = sat;
        });
        ...
        sub.on('realTime').atFrequency(2).handle((_t) => {
            ...
            if (isActive) {
                ...
                const eprVal = getEprFromN1Value(this.autoThrustLimit);
                const n1Limit = eprVal.toFixed(3).split('.', 2);
        ...
        return (
            <Layer x={698} y={28}>
                <g visibility={this.inactiveVisibility}>
                    <text class="Large Center Amber" x={0} y={0}>XX</text>
                    <text class="Large Center Amber" x={0} y={28}>XX</text>
                </g>
                <g visibility={this.activeVisibility}>
                    <text class="Huge Center Cyan" x={0} y={-1}>{this.textThrustLimitType}</text>
                    <text class="Huge End Green" x={-13} y={28}>{this.textThrustLimitInt}</text>
                    <text class="Large End Green" x={-4} y={28}>.</text>
                    <text class="Standard End Green" x={38} y={28}>{this.textThrustLimitFract}</text>
