...
export class NXDataStore {
    ...
    static get(key: string, defaultVal?: string): any {
        const val = GetStoredData(`A321EC_${key}`);
    ...
    static set(key: string, val: string): void {
        SetStoredData(`A321EC_${key}`, val);
        this.listener.triggerToAllSubscribers('A321EC_NXDATASTORE_UPDATE', key, val);
    }

    static subscribe(key: string, callback: SubscribeCallback): SubscribeCancellation {
        return Coherent.on('A321EC_NXDATASTORE_UPDATE', (updatedKey: string, value: string) => {
