...
impl PackFlowControllers<3, 2> for A318AirConditioning {
...
impl SimulationElement for A318AirConditioning {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
        self.a318_cabin.accept(visitor);
...
impl A318Cabin {
    ...
    const A318_CABIN_VOLUME_CUBIC_METER: f64 = 200.; // m3
    const A318_COCKPIT_VOLUME_CUBIC_METER: f64 = 10.; // m3
        ...
        Self {
            cabin_zone: [
                CabinZone::new(
                    context,
                    ZoneType::Cockpit,
                    Volume::new::<cubic_meter>(Self::A318_COCKPIT_VOLUME_CUBIC_METER),
                    2,
                    None,
                ),
                CabinZone::new(
                    context,
                    ZoneType::Cabin(1),
                    Volume::new::<cubic_meter>(Self::A318_CABIN_VOLUME_CUBIC_METER / 2.),
                    0,
                    Some([(1, 6), (7, 13)]),
                ),
                CabinZone::new(
    ...
    fn update(
        ...
        let flow_rate_per_cubic_meter: MassRate = MassRate::new::<kilogram_per_second>(
            pack_flow.pack_flow().get::<kilogram_per_second>()
                / (Self::A318_CABIN_VOLUME_CUBIC_METER + Self::A318_COCKPIT_VOLUME_CUBIC_METER),
        );
...
impl CabinTemperature for A318Cabin {
...
impl SimulationElement for A318Cabin {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
